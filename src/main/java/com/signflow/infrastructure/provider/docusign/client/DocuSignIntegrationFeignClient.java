...
public interface DocuSignIntegrationFeignClient {
    ...
    DocuSignEnvelopeResponseDTO updateEnvelope(
            @PathVariable String envelopeId,
            @RequestBody DocuSignUpdateEnvelopeDTO request);
    ...
    DocuSignRecipientsResponseDTO addRecipients(
            @PathVariable String envelopeId,
            @RequestBody DocuSignRecipientsDTO request);

    @PutMapping("/envelopes/{envelopeId}/recipients")
    DocuSignRecipientsResponseDTO resendToRecipients(
            @PathVariable String envelopeId,
            @RequestParam("resend_envelope") boolean resend,
            @RequestBody DocuSignRecipientsDTO request);
    ...
    DocuSignEnvelopeResponseDTO addDocuments(
            @PathVariable String envelopeId,
            @RequestBody DocuSignDocumentsUpdateDTO request);

    @PostMapping("/envelopes/{envelopeId}/recipients/{recipientId}/tabs")
    DocuSignTabsResponseDTO addTabs(
            @PathVariable String envelopeId,
            @PathVariable String recipientId,
            @RequestBody DocuSignTabDTO request);

    @PostMapping("/envelopes/{envelopeId}/recipients")
    DocuSignRecipientsResponseDTO addCarbonCopies(
            @PathVariable String envelopeId,
            @RequestBody DocuSignRecipientsDTO request);
}
