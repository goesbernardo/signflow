...
public class SignatureController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Envelope criado com sucesso",
                    content = @Content(schema = @Schema(implementation = Envelope.class),
                            examples = @ExampleObject(value = "{ \"externalId\": \"7db6f70a-0683-4a6c-9a4d-0453715c9298\", \"name\": \"Contrato de Aluguel\", \"status\": \"ACTIVE\", \"created\": \"2024-05-01T14:18:00Z\" }"))),
    ...
    public ResponseEntity<Envelope> createEnvelope(
            @Parameter(description = "Provedor de assinatura", example = "CLICKSIGN") @RequestHeader("provider") ProviderSignature provider,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(examples = @ExampleObject(value = "{ \"name\": \"Contrato de Prestação de Serviços\" }")))
            @RequestBody @Valid CreateEnvelopeCommand command) {
    ...
    public ResponseEntity<Envelope> getEnvelope(
            @Parameter(description = "Provedor de assinatura", example = "CLICKSIGN") @RequestHeader("provider") ProviderSignature provider,
    ...
    public ResponseEntity<Signer> addSigner(
            @Parameter(description = "Provedor de assinatura", example = "CLICKSIGN") @RequestHeader("provider") ProviderSignature provider,
            @PathVariable String externalId,
    ...
    public ResponseEntity<Void> addRequirement(
            @Parameter(description = "Provedor de assinatura", example = "CLICKSIGN") @RequestHeader("provider") ProviderSignature provider,
            @PathVariable String externalId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(examples = @ExampleObject(value = "{ \"documentExternalId\": \"doc-123\", \"signerExternalId\": \"signer-456\" }")))
    ...
    public ResponseEntity<Void> activateEnvelope(
            @Parameter(description = "Provedor de assinatura", example = "CLICKSIGN") @RequestHeader("provider") ProviderSignature provider,
            @PathVariable String externalId) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Timeline recuperada com sucesso",
                    content = @Content(mediaType = "application/json",
                            examples = @ExampleObject(value = "[{ \"previousStatus\": \"PROCESSING\", \"newStatus\": \"ACTIVE\", \"source\": \"SYSTEM\", \"occurredAt\": \"2024-05-01T14:18:00\" }]"))),
            @ApiResponse(responseCode = "401", description = "Não autorizado"),
            @ApiResponse(responseCode = "404", description = "Envelope não encontrado")
    })
